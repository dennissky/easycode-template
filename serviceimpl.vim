...
public class $!{tableName} implements $!{tableInfo.name}Service {
     ...
     */
    @Override
    public PageInfo<$!{tableInfo.name}PO> queryAllByLimit($!{tableInfo.name}PO $!tool.firstLowerCase($!{tableInfo.name}), int offset, int limit) {
        return PageHelper.startPage(offset, limit).doSelectPageInfo(()->$!{tool.firstLowerCase($!{tableInfo.name})}Dao.queryAll($!tool.firstLowerCase($!{tableInfo.name})));
    }
